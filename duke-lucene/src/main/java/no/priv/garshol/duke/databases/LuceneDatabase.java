...
/**
 * Represents the Lucene index, and implements record linkage services on top of
 * it.
 */
public class LuceneDatabase implements Database {
	...
	/**
	 * Controls whether to use fuzzy searches for properties that have fuzzy
	 * comparators. True by default.
	 */
	...
	/**
	 * Returns the path to the Lucene index directory. If null, it means the Lucene
	...
	/**
	 * The path to the Lucene index directory. If null or not set, it means the
	 * Lucene index is kept in-memory.
	 */
	...
	/**
	 * Tells the database to boost Lucene fields when searching for candidate
	 * matches, depending on their probabilities. This can help Lucene better pick
		...
		for (String propname : record.getProperties()) {
			...
			if (prop == null)
				throw new DukeConfigException(
						"Record has property " + propname + " for which there is no configuration");
...
//
//				doc.add(new StoredField(propname, v));

			} else {

				// NOT WORKING WITH LUCENCE 6.x
//				Field ix;
//				if (prop.isIdProperty())
//					ix = StringField(); // so findRecordById will work
...
//					ix = Field.Index.ANALYZED;
				...
				for (String v : record.getValues(propname)) {
					...
					Field field;
					if (prop.isIdProperty())
						field = new StringField(propname, v, Store.YES);
					else
						field = new TextField(propname, v, Store.YES);
...
//			}
		...
		for (Property prop : config.getLookupProperties()) {
			...
			for (String value : values)
				parseTokens(query, prop.getName(), value, prop.getLookupBehaviour() == Property.Lookup.REQUIRED,
						prop.getHighProbability());
		}

		// do the query
		return maintracker.doQuery(query.build());
	}
	...
	public String toString() {
		return "LuceneDatabase, max-search-hits: " + max_search_hits + ", min-relevance: " + min_relevance + ", fuzzy: "
				+ fuzzy_search + ", boost-mode: " + boost_mode + ", path: " + path + "\n  " + directory;
	}
	 ...
	 */
	private void parseTokens(BooleanQuery.Builder parent, String fieldName, String value, boolean required,
			double probability) {
...
//					termQuery.setBoost(boost);
	...
	/**
	 * The tracker is used to estimate the size of the query result we should ask
	 * Lucene for. This parameter is the single biggest influence on search
	 * performance, but setting it too low causes matches to be missed. We therefore
	 * try hard to estimate it as correctly as possible.
	 *
	 * The tracker uses a ring buffer of recent result sizes to estimate the result
	 * size.
	 */
	class EstimateResultTracker {
		private int limit;
		/**
		 * Ring buffer containing n last search result sizes, except for searches which
		 * found nothing.
		 */
		...
		public Collection<Record> doQuery(Query query, Integer filter) {
			List<Record> matches;
			try {
				ScoreDoc[] hits;
	...
	/**
	 * Checks to see if we need the spatial support, and if so creates the necessary
	 * context objects.
	 */
...
//		geoprop = new GeoProperty(prop);
	...
	public enum BoostMode {
		...
		QUERY,
		/**
		 * Boost fields at index time. This means records must be reindexed to change
		 * the boosting.
		 */
		INDEX,
		/**
